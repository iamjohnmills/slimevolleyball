class PatheticWhiteSlime extends SlimeAI {
  setSlimeMovement(options){
    ...
    // const rng = slimeRNG(options)
    // if(slimePowerJumps(options)) return;
    ...
    if( state != -1 || (options.ball.xv == 0 && options.ball.x == 800)) {
      if (state == -1) { // ball on player side
        ...
        if(Math.random() < 0.35) {
          state = Math.trunc(2 * Math.random()); // I think this will always return 2
        }
      }
  ...
  slimeRNG(){
    // RANDOM NUMBER LOGIC
    // slime jumping and close to net ELSE slime not jumping nor close to net
    return this.y > 0 && this.x < 575 ? 0 : 25 + Math.trunc(10 * Math.random())
  }
  slimePowerJumps(){
